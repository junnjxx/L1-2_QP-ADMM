% 读取 CSV 文件为矩阵 Y
bs =250 ;
csv_file = 'cifar10_matrix_Y_50000_250_HOG.csv';  % 替换为实际的文件路径
...
% Step 3: 保存调整后的矩阵
output_csv_file = 'adjusted_Y_cifar10_matrix_50000_250_HOG.csv';  % 替换为实际的输出文件路径
...
% 确保索引的合法性
n_sort = 50000; % 只对前 50000 条路径进行排序
if length(row_indices) < n_sort
    error('索引数量不足以覆盖需要排序的路径数量。');
end

if n_sort > length(X)
    error('X 文件中的路径数量少于 50000，请检查输入文件。');
end
...
% 保存到新的文件
fileID = fopen('cifar10_sorted_matrix_50000_250_HOG.txt', 'w');
fprintf(fileID, '%s\n', X_final{:});
fclose(fileID);

disp('路径排序完成，已保存至 cifar10_sorted_matrix_50000_250_HOG.txt');
%% sorted sample order of vector
csv_file = "sample_order_vector_1125_cifar10_50000_250_HOG.txt";  % 替换为实际的文件路径
row_indices = readmatrix(csv_file); 
row_indices = int32(row_indices+1);
% 读取图片路径文件
fileID = fopen('train_gray.txt', 'r');
...
% 确保索引的合法性
n_sort = 50000; % 只对前 30000 条路径进行排序
if length(row_indices) < n_sort
    error('索引数量不足以覆盖需要排序的路径数量。');
end
...
% 保存到新的文件
fileID = fopen('cifar10_sorted_vector_50000_250_HOG.txt', 'w');
fprintf(fileID, '%s\n', X_final{:});
fclose(fileID);

disp('路径排序完成，已保存至 cifar10_sorted_vector_50000_250_HOG.txt');
